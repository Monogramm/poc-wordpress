...
class eventsCrud extends event_controller
{
    ...
    public function add_new_entry($data, $events_table_name, $tags_table_name)
    {
        if (is_array($data))
        {
            ...
            global $wpdb;
            // preparing data
            $sql = $wpdb->prepare("INSERT INTO `$events_table_name`      
                           (`title`,`description`,`image`,`date`,`start_time`,`end_time`,`event_category`) 
                            values (%s, %s, %s, %s, %s, %s, %s) ", $data['title'], $data['description'], $data['image'], $data['date'], $data['start_time'], $data['end_time'], $data['event_category']);
            // insert data
            ...
            if (false !== $result)
            {
                ...
                */
                ...
                if (is_array($tagsArray))
                {
                    ...
                    {
                        $sqlTags = $wpdb->prepare("INSERT INTO `$tags_table_name` (`event_id`,`tag`) values (%s, %s) ", $event_id, $tagsArray[$i]);
                        $wpdb->query($sqlTags);
                    }
                }
                return true;
    ...
    public function update_entry($data, $id, $events_table_name, $tags_table_name)
    {
        if (is_array($data))
        {
            ...
            global $wpdb;
            // preparing data
            $sql = $wpdb->prepare("UPDATE `$events_table_name`      
                           SET `title` = %s,`description` = %s,`image` = %s,`date` = %s,
                                `start_time` = %s,`end_time` = %s, `event_category` = %s
                                WHERE `id` = $id
                            ", $data['title'], $data['description'], $data['image'], $data['date'], $data['start_time'], $data['end_time'], $data['event_category']);
            // Update data
            $result = $wpdb->query($sql);
            ...
            if (false !== $result)
            {
                ...
                */
                ...
                if (is_array($tagsArray))
                {
                    for ($i = 0;$i < count($tagsArray);$i++)
                    {
                        $sqlTags = $wpdb->prepare("INSERT INTO `$tags_table_name` (`event_id`,`tag`) values (%s, %s) ", $id, $tagsArray[$i]);
                        $wpdb->query($sqlTags);
                    }
                }
                return true;
